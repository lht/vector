...
mod tests {
    ...
    #[test]
    fn test_kinesis_stream_record_creation() {
        let payload = Bytes::from("test data");
        let partition_key = "test-partition";

        let record = KinesisStreamRecord::new(&payload, partition_key);

        assert_eq!(record.record.partition_key(), partition_key);
        assert_eq!(record.record.data().as_ref(), payload.as_ref());
    }

    #[test]
    fn test_kinesis_stream_record_encoded_length() {
        let payload = Bytes::from("test data");
        let partition_key = "test-partition";

        let record = KinesisStreamRecord::new(&payload, partition_key);
        let length = record.encoded_length();

        // Length should be > 0 and account for base64 encoding and metadata
        assert!(length > 0);
        assert!(length > payload.len()); // Should be larger due to base64 encoding
    }
}
